...
- Payment Details: Bank: 9123565629, Provider: PALMPAY, Account Name: ETIM.`;
    ...
    try {
      const response = await fetch('/api/generate', {
        ...
        body: JSON.stringify({ contents: query, systemInstruction: sysInstruction }),
      });
      const data = await response.json();
      setTimeout(() => { 
        setMessages(prev => [...prev, { id: Date.now().toString() + '_ai', from: 'ai', text: data.text || "Neural link protocol error." }]); 
        setIsTyping(false); 
      }, 1200);
    } catch { 
      setMessages(prev => [...prev, { id: 'err', from: 'ai', text: "Neural link lost. Check terminal connection." }]); 
      setIsTyping(false); 
    }
  };
    ...
    <div className="fixed inset-0 z-[3000] bg-[#050C1F] flex flex-col animate-slide-up">
      <header className="px-6 py-6 border-b border-white/5 flex items-center justify-between bg-[#0A162B] shrink-0">
        ...
        <div className="text-center">
          <h3 className="text-sm font-black text-white uppercase tracking-widest">Secure Terminal</h3>
          <span className="text-[9px] text-[#00D775] font-bold uppercase tracking-widest">Encrypted Session</span>
        </div>
      ...
      <div className="p-4 bg-[#0A162B] border-t border-white/5 flex flex-col gap-3 pb-8">
        ...
        <input type="file" id="file_input" ref={fileInputRef} className="hidden" accept="image/*" onChange={e => { const f = e.target.files?.[0]; if (f) { const r = new FileReader(); r.onloadend = () => handleSend('file', r.result as string); r.readAsDataURL(f); } }} />
      </div>
...
const App: React.FC = () => {
                ...
                {!isOwned ? (
                  <div className="pt-2 flex flex-col items-center gap-4 relative">
                    ...
                    <div className="w-full relative">
                      <button onMouseEnter={() => setLockedHoverCardId(catalogCard.id)} onMouseLeave={() => setLockedHoverCardId(null)} className={`w-full py-4 mb-2 bg-[#111F35] text-white font-black text-[11px] tracking-widest uppercase rounded-2xl border transition-all duration-300 ${isHovered ? theme.glow + ' scale-[1.02]' : 'border-white/5'}`}>See Card Details (Locked)</button>
                    </div>
                    <button onClick={() => setPurchaseChatProps({ initialMsg: "I WANT TO PURCHASE THIS CARD", purchaseType: 'card', purchaseContext: { item: catalogCard.label, price: catalogCard.price, theme: catalogCard.theme }})} className="w-full py-4 bg-white text-black font-black text-[11px] tracking-[0.3em] uppercase rounded-2xl active:scale-95 transition-all shadow-xl">Buy Now</button>
                  </div>
                ) : <div className="pt-2 text-center"><p className="text-[10px] font-bold text-[#00D775] uppercase tracking-widest">Card successfully issued to {userName}</p></div>}
              </div>
            );
          })}
        </div>
  ...
  const renderServicesPage = () => {
    if (selectedInboxNumber) {
        ...
        return (
            <div className="flex flex-col h-screen bg-[#050C1F] animate-fade-in overflow-hidden relative">
                <header className="px-6 py-10 bg-[#0A162B] border-b border-white/5 flex items-center justify-between shrink-0"><button onClick={() => setSelectedInboxNumber(null)} className="p-2 -ml-2 text-white/40 hover:text-white transition-colors"><ArrowLeftIcon className="w-6 h-6" /></button><div className="text-center"><div className="flex items-center gap-2 justify-center mb-1"><span className="text-sm font-black text-white">{selectedInboxNumber.number}</span></div><p className="text-[10px] text-white/30 font-bold uppercase tracking-[0.2em]">{daysLeft} Days Remaining</p></div><div className="w-8"></div></header>
                <div className="flex-grow overflow-y-auto p-6 space-y-4 scrollbar-hide bg-[#050C1F]">
                    {msgs.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-center opacity-20"><ChatBubbleIcon className="w-16 h-16 mb-4" /><p className="text-xs font-black uppercase tracking-[0.3em]">No incoming messages</p></div> : msgs.map(m => (<div key={m.id} className={`flex ${m.from === 'admin' || m.from === 'system' ? 'justify-start' : 'justify-end'}`}><div className={`max-w-[85%] p-4 rounded-[24px] ${m.from === 'admin' || m.from === 'system' ? 'bg-[#1E293B]/50 border border-white/10' : 'bg-[#5EB5FB] text-black font-medium'}`}><p className="text-sm whitespace-pre-wrap">{m.content}</p></div></div>))}
                </div>
            </div>
        );
    }
    return (
      <div className="flex flex-col h-screen bg-[#050C1F] animate-fade-in overflow-hidden relative">
        <header className="px-6 py-10 border-b border-white/5 flex items-center justify-between shrink-0"><div><h2 className="text-2xl font-black tracking-tight uppercase leading-none">SERVICES</h2><p className="text-[#5EB5FB] text-[10px] font-black uppercase mt-1.5 tracking-widest">PHONE NUMBERS</p></div></header>
        <div className="px-6 pt-2 pb-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {currentUser?.boughtNumbers?.map(num => (
                            <button key={num.id} onClick={() => setSelectedInboxNumber(num)} className="w-full bg-[#111F35] border border-white/10 rounded-2xl p-4 flex justify-between items-center active:scale-[0.98] transition-all group">
                                <div className="flex items-center gap-4"><div className="w-10 h-10 rounded-xl flex items-center justify-center bg-[#00D775]/10 text-[#00D775] border border-[#00D775]/20"><span className="text-lg">{num.countryFlag}</span></div><div className="text-left"><p className="text-[13px] font-black text-white tracking-tight">{num.number}</p></div></div>
            ...
            <div className="relative"><SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/20" /><input value={phoneSearch} onChange={e => setPhoneSearch(e.target.value)} placeholder="Filter countries..." className="w-full bg-[#111F35] border border-white/5 rounded-2xl py-4 pl-12 pr-4 text-xs text-white outline-none focus:border-[#5EB5FB]/40 transition-all font-bold" /></div>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="fixed inset-0 z-[1500] bg-[#0066F5] flex flex-col items-center overflow-y-auto scrollbar-hide">
        <div className="w-full max-w-md px-6 py-10 flex flex-col items-center shrink-0 relative z-10">
          <div className="flex items-center gap-2 mb-8"><div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-lg"><span className="text-[#0066F5] font-black text-2xl">M</span></div><h1 className="text-white font-black text-2xl leading-none tracking-tighter">Moniepoint</h1></div>
          <div className="w-full bg-white rounded-t-[32px] pt-8 pb-1 flex flex-col relative shadow-2xl">
            <div className="px-8 mb-4"><span className="inline-block bg-[#E6F0FF] text-[#0066F5] text-[10px] font-black px-2.5 py-1 rounded-md mb-3 tracking-widest uppercase">{lastTx?.status === 'PENDING' ? 'Processing' : 'Debit'}</span><h2 className="text-[36px] font-black text-[#050C1F] leading-none mb-1">₦{parseFloat(transferAmount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</h2></div>
            <div className="bg-[#F8FAFF] mx-4 rounded-[24px] p-6 space-y-6">
              <div><p className="text-[11px] font-medium text-gray-400 mb-2">Recipient</p><p className="text-[13px] font-bold text-gray-900 uppercase">{verifiedAccountName}</p></div>
              <div><p className="text-[11px] font-medium text-gray-400 mb-2">Institution</p><p className="text-[13px] font-bold text-gray-900 uppercase">{selectedBank?.name}</p></div>
            </div>
          </div>
          <div className="w-full max-md px-4 mt-8 space-y-4 relative z-10 mb-20"><button onClick={() => { setActiveTab(AppTab.HOME); setTransferStep('recipient'); setTransferAmount(""); }} className="w-full py-4 rounded-xl bg-white text-[#0066F5] font-black text-sm tracking-widest uppercase shadow-xl">Done</button></div>
        </div>
      </div>
    );
  };
  ...
  const renderTransfer = () => {
    if (!ProtectionModule.checkAccess(isAuthenticated, "TransferPage")) return null;
    if (transferStep === 'receipt') return renderReceipt();
    if (isValidating) return renderValidatingScreen();
    if (transferStep === 'amount') return renderAddAmount();
    if (transferStep === 'summary') return renderSummary();

    const filteredBanks = NIGERIAN_BANKS.filter(bank => bank.name.toLowerCase().includes(bankSearchQuery.toLowerCase()));
    const isContinueEnabled = receiverAccount.length === 10 && selectedBank !== null && verifiedAccountName.length > 0;

    return (
      <div className="flex flex-col h-screen bg-[#050C1F] overflow-hidden animate-fade-in">
        <div className="px-4 py-3 flex items-center justify-between border-b border-white/5 bg-[#050C1F] shrink-0">
          <button onClick={() => { setActiveTab(AppTab.HOME); setReceiverAccount(""); setSelectedBank(null); }} className="p-1 -ml-1">
            <ArrowLeftIcon className="w-5 h-5" />
          </button>
          <h2 className="text-[13px] font-bold tracking-tight">Transfer</h2>
          <div className="w-8"></div>
        </div>
        <div className="flex-grow overflow-y-auto px-4 py-4 space-y-4 pb-48 scrollbar-hide">
          <div className="space-y-1.5">
            <p className="text-[10px] text-white/30 font-bold uppercase tracking-tighter px-1">Receiver's account number</p>
            <div className={`bg-[#111F35] rounded-xl p-3 border transition-all ${receiverAccount.length > 0 ? 'border-[#FBC02D]/40' : 'border-white/5'}`}>
              <input type="text" value={receiverAccount} onChange={(e) => { const val = e.target.value.replace(/[^0-9]/g, ''); if (val.length <= 10) setReceiverAccount(val); }} className="bg-transparent text-[16px] font-bold text-white w-full outline-none" autoFocus />
            </div>
          </div>
          {isVerifying && !verifiedAccountName && <VerifyingAccountWidget verifyAccount={verifyAccountApi} onComplete={(name) => setVerifiedAccountName(name)} onError={() => {}} />}
          {verifiedAccountName && (
            <div className="bg-[#111F35] p-3 rounded-xl border border-white/5 flex items-center gap-3 animate-fade-in shadow-lg">
              <div className="w-10 h-10 bg-[#1D2B44] rounded-full flex items-center justify-center font-bold text-white/30 text-[11px]">{verifiedAccountName.substring(0, 2).toUpperCase()}</div>
              <div className="flex-grow min-w-0">
                <h4 className="font-bold text-[12px] text-white/90 truncate uppercase tracking-tight">{verifiedAccountName}</h4>
                <p className="text-[10px] text-white/30 font-medium truncate">{selectedBank?.name}</p>
              </div>
            </div>
          )}
          <div className="space-y-3 pt-1">
            <div className="bg-[#111F35] flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-white/5 sticky top-0 z-10">
              <SearchIcon className="w-4 h-4 text-white/40" />
              <input type="text" placeholder="Search bank..." value={bankSearchQuery} onChange={(e) => setBankSearchQuery(e.target.value)} className="bg-transparent flex-grow text-[11px] outline-none" />
            </div>
            <div className="bg-[#111F35] rounded-xl border border-white/5 overflow-hidden divide-y divide-white/5">
              {filteredBanks.slice(0, 10).map((bank) => (
                <button key={bank.code} onClick={() => setSelectedBank(bank)} className={`w-full flex items-center justify-between p-4 hover:bg-white/5 transition-colors ${selectedBank?.code === bank.code ? 'bg-[#5EB5FB]/5' : ''}`}>
                  <span className={`font-bold text-[11px] ${selectedBank?.code === bank.code ? 'text-[#5EB5FB]' : 'text-white/80'}`}>{bank.name}</span>
                </button>
              ))}
            </div>
          </div>
        </div>
        <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-6 bg-[#050C1F] z-[200]">
          <button disabled={!isContinueEnabled} onClick={() => setIsConfirmingRecipient(true)} className={`w-full py-4 rounded-xl font-black text-[13px] tracking-widest uppercase transition-all shadow-2xl ${isContinueEnabled ? 'bg-[#5EB5FB] text-[#050C1F]' : 'bg-[#111F35] text-white/10'}`}>Continue</button>
        </div>
        {isConfirmingRecipient && (
          <div className="fixed inset-0 z-[300] flex flex-col justify-end">
            <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setIsConfirmingRecipient(false)}></div>
            <div className="relative bg-[#0A1A2F] rounded-t-[32px] p-6 pt-12 pb-10 flex flex-col items-center animate-slide-up shadow-2xl">
              <div className="text-center mb-8 w-full">
                <p className="text-[12px] text-white/40 font-medium mb-2 tracking-tight">Sending money to</p>
                <h3 className="text-[20px] font-bold text-white mb-1.5 tracking-tight">{verifiedAccountName}</h3>
              </div>
              <button onClick={() => { setIsConfirmingRecipient(false); setTransferStep('amount'); }} className="w-full bg-[#FBC02D] text-[#050C1F] py-4 rounded-xl font-bold text-[14px]">Confirm Recipient</button>
            </div>
          </div>
        )}
      </div>
  ...
  const renderAddAmount = () => (<div className="flex flex-col h-screen bg-[#050C1F] overflow-hidden animate-fade-in relative"><div className="px-4 py-3 flex items-center justify-between shrink-0"><button onClick={() => setTransferStep('recipient')} className="p-1 -ml-1"><ArrowLeftIcon className="w-5 h-5" /></button><h2 className="text-[13px] font-bold tracking-tight">Add amount</h2><div className="w-8"></div></div><div className="flex flex-col items-center mb-6 px-6 mt-20"><div className="flex items-center justify-center"><span className="text-[28px] font-bold text-white mr-1">₦</span><input type="text" inputMode="numeric" value={transferAmount} onChange={handleAmountChange} className="bg-transparent text-[36px] font-bold text-white outline-none w-auto max-w-[180px] text-center" autoFocus /></div></div><div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto px-6 pb-6"><button disabled={parseInt(transferAmount) < 100} onClick={startValidation} className={`w-full py-3.5 rounded-xl font-bold text-[14px] transition-all transform active:scale-95 ${parseInt(transferAmount) >= 100 ? 'bg-[#1E293B] text-[#FBC02D]' : 'bg-[#111F35] text-[#333E4D]'}`}>Continue</button></div></div>);
  const renderSummary = () => (<div className="flex flex-col h-screen bg-[#050C1F] overflow-hidden animate-fade-in relative"><div className="px-4 py-3 flex items-center justify-between shrink-0"><button onClick={() => setTransferStep('amount')} className="p-1 -ml-1 text-white/80"><ArrowLeftIcon className="w-5 h-5" /></button><h2 className="text-[14px] font-bold tracking-tight text-white">Confirm payment</h2><div className="w-8"></div></div><div className="flex flex-col items-center mt-10 px-6"><h3 className="text-[36px] font-bold text-white tracking-tight leading-none mb-3">₦{parseFloat(transferAmount).toLocaleString()}</h3></div><div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto px-4 pb-8 pt-4 bg-[#050C1F] z-[100]"><button onClick={handlePaymentConfirm} className="w-full py-4 rounded-xl bg-[#FBC02D] text-[#050C1F] font-bold text-[15px]">Pay</button></div></div>);
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#050C1F] text-white relative flex flex-col antialiased font-sans">
      ...
      {pinMode !== 'inactive' && (<PinOverlay title={pinMode === 'verify' ? 'Enter PIN' : 'Setup PIN'} error={pinError} onComplete={handlePinComplete} onCancel={() => setPinMode('inactive')} />)}
      ...
      <AITextAssistant userId={currentUserWallet} balance={balance} userName={userName} deviceType="Mobile Terminal" />
      ...
      {isProfileOpen && (
        <div className="fixed inset-0 z-[200] flex flex-col justify-end">
          ...
          <div className="relative bg-[#050C1F] rounded-t-[32px] p-6 pb-12 animate-slide-up border-t border-white/10 max-h-[85vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold tracking-tight">Profile</h2><button onClick={handleLogout} className="text-red-500 font-bold text-sm bg-red-500/10 px-4 py-2 rounded-full">Logout</button></div>
            <div className="flex flex-col items-center mb-8"><div className="w-24 h-24 bg-[#111F35] rounded-full flex items-center justify-center border-4 border-white/5 overflow-hidden"><span className="text-3xl font-black text-[#FBC02D]">{userName.substring(0, 2).toUpperCase()}</span></div><h3 className="text-lg font-black tracking-tight mt-4">{userName}</h3></div>
            <button onClick={() => setIsProfileOpen(false)} className="w-full bg-[#5EB5FB] text-[#050C1F] font-bold py-4 rounded-2xl">Close Profile</button>
          </div>
        </div>
      )}
      <style>{`
